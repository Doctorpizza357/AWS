...
│                                                                             │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐  │
│  │   aws4      │  │   multer    │  │  pdf-parse   │  │  JSON parsing   │  │
│  │  SigV4 sign │  │  PDF upload │  │  text extract│  │  + normalization│  │
│  └──────┬──────┘  └─────────────┘  └──────────────┘  └─────────────────┘  │
│         │                                                                    │
...
└────────┬─────────┘
         ...
         ├──► fetchSalaryData(id)   ──► BLS API v2 (p10/p25/median/p75/p90, national)
...
└────────────────────────────────────────────────────────────────────┘
   ...
   │ ATS scoring     │    │ Response analysis│    │ Code review     │
   │ Keyword gaps    │    │ Follow-ups       │    │ Complexity      │
   │ Optimized resume│    │ Scoring          │    │ Suggestions     │
   └─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
• User profile + progress: Written to Firestore on change, cached in localStorage
